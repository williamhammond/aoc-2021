...
int find_lowest_points(const std::vector<std::vector<int>>& grid) {
  ...
  for (size_t i = 0; i < grid.size(); i++) {
    for (size_t j = 0; j < grid[i].size(); j++) {
      const bool is_left_higher = (j - 1 >= 0) || grid[i][j - 1] > grid[i][j];
      const bool is_right_higher =
          (j + 1 == grid[i].size()) || grid[i][j + 1] > grid[i][j];
      const bool is_up_higher = (i - 1 >= 0) || grid[i - 1][j] > grid[i][j];
...
uint32_t flood_fill(std::vector<std::vector<int>>& grid, uint32_t initial_row,
  ...
  if (initial_row >= grid.size() ||
      initial_column >= grid[initial_row].size() ||
  ...
  while (!queue.empty()) {
    ...
    if (current_column + 1 < grid[current_row].size() &&
