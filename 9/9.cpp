...
uint64_t solve(const std::vector<std::vector<int>>& grid)
{
	...
	for (int i = 0; i < grid.size(); i++)
	{
		for (int j = 0; j < grid[i].size(); j++)
		{
			...
			const bool is_right_higher = (j + 1 == grid[i].size()) || grid[i][j + 1] > grid[i][j];
			const bool is_up_higher = (i - 1 == -1) || grid[i - 1][j] > grid[i][j];
			const bool is_down_higher = (i + 1 == grid.size()) || grid[i + 1][j] > grid[i][j];
			const bool is_low_point = is_left_higher && is_right_higher && is_up_higher && is_down_higher;
